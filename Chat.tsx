...
export default function Chat({ messages, onSendMessage, onClearChat, onDeleteMessage, onEditMessage, onSetDisappear, disappearAfter, currentUserId, className = '' }: ChatProps) {
  ...
  const handleMessagesScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (top < lastScrollTop.current - 4) setHeaderVisible(true)   // scrolling up
    else if (top > lastScrollTop.current + 4) setHeaderVisible(false) // scrolling down
  ...
  return (
    <div className={`flex flex-col ${className}`} style={{ backgroundColor: '#000000' }}>
      ...
      {/* Header — always visible on desktop; slides in on mobile when scrolling up */}
      <div className={`px-4 py-3 border-b border-gray-700/50 items-center gap-2 shrink-0 relative ${headerVisible ? 'flex md:flex' : 'hidden md:flex'}`} >
      ...
      <div className="p-3 border-t border-gray-700/50 shrink-0">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className="p-2.5 bg-green-400 hover:bg-green-500 disabled:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed text-black rounded-xl transition-colors shrink-0"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="currentColor" viewBox="0 0 16 16">
