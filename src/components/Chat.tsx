      ...
      <div className="absolute" style={{ top: `${pct}%`, left: 0, right: 0, transform: 'translateY(-50%)' }}>
        ...
        <div
          ...
          style={{
            ...
            backgroundColor: peerActive ? '#c61700' : '#ff5722',
          }}
        />
      </div>
    </div>
  ...
  return (
    <div className={`flex flex-col ${className}`} style={{ backgroundColor: '#000000' }}>
      ...
      <div id="input-area" className="relative p-3 border-t border-gray-700/50 shrink-0">
        {/* Floating clear chat and code icon */}
        <button
          onClick={() => {
            onPanicWipe?.();
            try { navigator.vibrate?.(50) } catch {}
          }}
          title="Clear chat and code instantly"
          aria-label="Clear chat and code"
          className="opacity-75 fixed top-1/2 -translate-y-1/2 left-3 w-9 h-9 rounded-full bg-black/30 backdrop-blur-sm border-2 border-gray-500/50 hover:border-gray-400/60 hover:bg-black/40 shadow-lg hover:scale-110 transition-all z-10 flex items-center justify-center"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="rgba(107, 114, 128, 0.8)" viewBox="0 0 16 16">
