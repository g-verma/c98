...
export default function Toolbar({ roomId, displayName, language, userCount, userName, connected, onLanguageChange, onClearCode, onRenameUser, onLogout, currentPassword, onChangePassword, onExitAll }: ToolbarProps) {
  ...
  return (
    <>
      ...
      {onExitAll && (
        <button
          onClick={onExitAll}
          title="Remove all other users from room"
          className="flex items-center justify-center w-6 h-6 text-gray-500 hover:text-red-400 transition-colors max-w-[10px]"
        >
      ...
      <div className="flex items-center gap-2 shrink-0">
        {editingName ? (
          ...
          <button
            ...
            className="flex items-center !w-auto !max-w-[70px] !h-auto gap-1 text-xs text-gray-500 hover:text-gray-300 max-w-[130px] truncate transition-colors"
          >
            👤 {userName}
